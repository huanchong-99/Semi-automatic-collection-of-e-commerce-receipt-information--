...
class DataProcessor:
    ...
    def _find_element_smart(self, name, original_xpath):
        ...
        if self.coordinate_cache.should_use_cache():
            try:
                cached_coords_tuple = self.coordinate_cache.get_cached_coordinate(name)
                if cached_coords_tuple:
                    cached_coords = {'x': cached_coords_tuple[0], 'y': cached_coords_tuple[1]}
                    self._log_info(f"重试模式：尝试使用缓存坐标查找元素 '{name}': {cached_coords}", "orange")
                    # 创建虚拟元素对象，包含缓存的坐标信息
                    virtual_element = self._create_virtual_element(name, cached_coords)
                    if virtual_element:
    ...
    def _create_virtual_element_enhanced(self, name, cached_coords):
        ...
        try:
            ...
            virtual_element = EnhancedVirtualElement(name, cached_coords)
            self._log_info(f"成功创建增强虚拟元素 '{name}' 使用坐标: ({cached_coords['screen_x']}, {cached_coords['screen_y']})", "green")
